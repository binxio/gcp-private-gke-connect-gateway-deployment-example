...
resource "google_gke_hub_membership" "example" {
  ...
  membership_id = "example-${random_id.example.hex}"
...
resource "google_service_account" "example" {
  project      = var.project_id
  account_id   = "example-${random_id.example.hex}"
...
resource "google_container_cluster" "example" {
  ...
  ip_allocation_policy {
    services_secondary_range_name = "example-${random_id.example.hex}-nl-gke-services"
    cluster_secondary_range_name  = "example-${random_id.example.hex}-nl-gke-pods"
  }
